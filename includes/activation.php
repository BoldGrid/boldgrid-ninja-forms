...
/* BEGIN: BoldGrid. */
        ...
        if ( ! defined( 'BG_NINJA_FORMS_FAV_FIELDS_TABLE_NAME' ) )
            define( 'BG_NINJA_FORMS_FAV_FIELDS_TABLE_NAME', $wpdb->prefix . 'ninja_forms_fav_fields' );
...
// BoldGrid modified constant names below.
		...
		$sql = "CREATE TABLE IF NOT EXISTS ".BG_NINJA_FORMS_FAV_FIELDS_TABLE_NAME." (
		...
		$state_dropdown = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM ".BG_NINJA_FORMS_FAV_FIELDS_TABLE_NAME." WHERE name = %s AND row_type = 0", 'State Dropdown' ), ARRAY_A );
		...
		$anti_spam = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM ".BG_NINJA_FORMS_FAV_FIELDS_TABLE_NAME." WHERE name = %s AND row_type = 0", 'Anti-Spam' ), ARRAY_A );
		...
		$submit = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM ".BG_NINJA_FORMS_FAV_FIELDS_TABLE_NAME." WHERE name = %s AND row_type = 0", 'Submit' ), ARRAY_A );
		if( !isset($submit['id']) ){
			$sql = 'INSERT INTO `'.BG_NINJA_FORMS_FAV_FIELDS_TABLE_NAME.'` (`id`, `row_type`, `type`, `order`, `data`, `name`) VALUES
		...
		$sql = "CREATE TABLE IF NOT EXISTS ".BG_NINJA_FORMS_FIELDS_TABLE_NAME." (
		  `id` int(11) NOT NULL AUTO_INCREMENT,
		...
		dbDelta($sql);
	 	...
	 	$sql = "CREATE TABLE IF NOT EXISTS ". BG_NF_OBJECT_META_TABLE_NAME . " (
		...
		$sql = "CREATE TABLE IF NOT EXISTS " . BG_NF_OBJECTS_TABLE_NAME . " (
		  `id` bigint(20) NOT NULL AUTO_INCREMENT,
		...
		$sql = "CREATE TABLE IF NOT EXISTS " . BG_NF_OBJECT_RELATIONSHIPS_TABLE_NAME . " (
		  `id` bigint(20) NOT NULL AUTO_INCREMENT,
		...
		// BoldGrid: set transient redirect to false to prevent loading NF info page after activation from inspirations.
		set_transient( '_nf_activation_redirect', false, 30 );
