	...
	public function __construct( $boldgrid_ninja_forms ) {
		...
		if ( is_admin() ) {
			...
			$is_plugin_information = ( 'plugin-install.php' === $pagenow && isset( $_GET['plugin'] ) &&
				 'boldgrid-ninja-forms' === $_GET['plugin'] && isset( $_GET['tab'] ) &&
	...
	public function custom_plugins_transient_update( $transient ) {
		...
		if ( $have_configs && ( false === $version_data || ( $is_force_check && $is_data_old ) ) ) {
			...
			if ( false === empty( $version_data ) && 200 === $version_data->status &&
				...
				if ( is_multisite() ) {
					delete_site_transient( 'boldgrid_ninja_forms_version_data' );
					set_site_transient( 'boldgrid_ninja_forms_version_data', $version_data,
						8 * HOUR_IN_SECONDS );
				} else {
					delete_transient( 'boldgrid_ninja_forms_version_data' );
					set_transient( 'boldgrid_ninja_forms_version_data', $version_data,
						8 * HOUR_IN_SECONDS );
				}
			} else {
		...
		if ( 'plugin-install.php' === $pagenow && isset( $_GET['plugin'] ) &&
			...
			$transient->download_link = $configs['asset_server'] .
				 ...
				 '&installed_form_version=' . $plugin_data['Version'] . '&installed_wp_version=' .
				 $wp_version;
