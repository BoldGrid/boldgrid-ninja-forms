 ...
 *
 * @package Boldgrid_Ninja_Forms_Update
...
class Boldgrid_Ninja_Forms_Update {
	 ...
	 *
	 * @param object $boldgrid_ninja_forms        	
	 *
	 * @return bool
	 */
	...
	public function custom_plugins_transient_update( $transient, $plugin_info = null, $plugin_info_obj = null ) {
		...
		if ( is_multisite() ) {
			$version_data = get_site_transient( 'boldgrid_ninja_form_version_data' );
		} else {
			$version_data = get_transient( 'boldgrid_ninja_form_version_data' );
		}
		...
		if ( ! empty( $version_data ) && 200 == $version_data->status &&
			...
			if ( is_multisite() ) {
				delete_site_transient( 'boldgrid_form_version_data' );
				set_site_transient( 'boldgrid_form_version_data', $version_data, 
					8 * HOUR_IN_SECONDS );
			} else {
				delete_transient( 'boldgrid_form_version_data' );
				set_transient( 'boldgrid_form_version_data', $version_data, 8 * HOUR_IN_SECONDS );
			}
		} else {
	...
	if ( 'plugin-install.php' == $pagenow && isset( $_GET['plugin'] ) &&
		 'boldgrid-ninja-forms' == $_GET['plugin'] ) {
		...
		$transient->download_link = $configs['asset_server'] . $configs['ajax_calls']['get_asset'] .
			 '?id=' . $version_data->result->data->asset_id . '&installed_form_version=' .
		...
		$transient->plugin_name = 'ninja-forms.php';
		$transient->slug = 'boldgrid-ninja-forms';
		...
		$obj = new stdClass();
		$obj->slug = 'boldgrid-ninja-forms';
		$obj->plugin = 'boldgrid-ninja-forms/ninja-forms.php';
		...
		$obj->package = $configs['asset_server'] . $configs['ajax_calls']['get_asset'] . '?id=' .
			 $version_data->result->data->asset_id . '&installed_form_version=' .
			 $plugin_data['Version'] . '&installed_wp_version=' . $wp_version;
